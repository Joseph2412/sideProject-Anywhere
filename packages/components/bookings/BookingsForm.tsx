...
export const BookingsForm: React.FC<VenueBookingsHistoryProps> = ({
  venueId: propVenueId,
}) => {
  ...
  const calculateStats = () => {
    ...
    const stats = {
      ...
      totalRevenue:
        bookings
          .filter(
            (b) => b.status === "COMPLETED" && typeof b.price === "number",
          )
  ...
  const columns = [
    {
      ...
      width: 70,
      render: (id: number) => (
        <span style={{ fontWeight: "bold", color: "#1890ff" }}>#{id}</span>
      ),
    },
    {
      title: "Cliente",
    ...
    },
    {
      ...
      render: (price: number) => (
        <span
          style={{ fontWeight: "bold", color: "#52c41a", fontSize: "16px" }}
        >
          €{price?.toFixed(2)}
        </span>
      ),
        ...
        <Table
          ...
          pagination={{
            ...
            showTotal: (total, range) =>
              `${range[0]}-${range[1]} di ${total} prenotazioni`,
