...
export const useGooglePlaces = () => {
  ...
  const searchPlaces = useCallback(async (input: string): Promise<GooglePrediction[]> => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error('Errore HTTP:', response.status, response.statusText, errorText);
  ...
  const getPlaceDetails = useCallback(async (placeId: string): Promise<PlaceResult | null> => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Errore nel recupero dettagli';
      setError(errorMessage);
