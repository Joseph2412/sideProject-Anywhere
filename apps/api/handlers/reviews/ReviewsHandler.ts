...
export const createReviewsHandler = async (
  request: BookingRequest,
  reply: FastifyReply,
) => {
  const { reviewWifi, reviewStaff, reviewQuiet, reviewComfort, reviewOverall } =
    request.body as {
      reviewWifi: number;
      reviewStaff: number;
      reviewQuiet: number;
...
export const getReviewsHandler = async (
  ...
  return reply
    .status(200)
    .send({ reviews, message: "Reviews fetched successfully" });
};
